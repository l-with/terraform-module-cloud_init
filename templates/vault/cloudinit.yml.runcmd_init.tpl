  ...
  - |
    if [ "$VAULT_INITIALIZED" = "false" ]; then
      ...
      tar -zc /root/vault_init.json | openssl enc -aes-256-cbc -pbkdf2 -out ${vault_init_artifact} -pass env:VAULT_ENCRYPT_SECRET
      s3cmd --access_key=${vault_s3_access_key} --secret_key=${vault_s3_secret_key} \
        ...
        put ${vault_init_artifact} s3://${vault_s3_bucket}%{ if vault_s3_prefix != null }/${vault_s3_prefix}%{ endif }
    fi
