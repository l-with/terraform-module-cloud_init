...
module "mailcow_needs_mailcow_admin_user_and_mailcow_admin_password" {
  source  = "rhythmictech/errorcheck/terraform"
  version = "~> 1.3.0"

  use_jq        = true
  assert        = !(var.mailcow && (var.mailcow_admin_user == null || var.mailcow_admin_password == null))
  error_message = "error: mailcow needs mailcow_admin_user and mailcow_admin_password"
}
