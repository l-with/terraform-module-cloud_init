...
s. [caddy](https://github.com/caddyserver/caddy)

For input variables: s. [caddy](#input_caddy).
...
| <a name="input_caddy"></a> [caddy](#input\_caddy) | if cloud-init user data for installing caddy should be generated | `bool` | `false` | no |
| <a name="input_caddy_configuration"></a> [caddy\_configuration](#input\_caddy\_configuration) | the caddy configuration | `string` | `null` | no |
...
| <a name="input_certbot_automatic_renewal_deploy_hooks"></a> [certbot\_automatic\_renewal\_deploy\_hooks](#input\_certbot\_automatic\_renewal\_deploy\_hooks) | the certbot automatic renewal deploy hook scripts | <pre>list(object({<br>    file_name = string<br>    content   = string<br>  }))</pre> | `[]` | no |
...
| <a name="input_certbot_automatic_renewal_pre_hooks"></a> [certbot\_automatic\_renewal\_pre\_hooks](#input\_certbot\_automatic\_renewal\_pre\_hooks) | the certbot automatic renewal pre hook scripts | <pre>list(object({<br>    file_name = string<br>    content   = string<br>  }))</pre> | `[]` | no |
...
| <a name="input_hetzner"></a> [hetzner](#input\_hetzner) | if cloud-init user data for making changes on a Hetzner Cloud Server should be generated | `bool` | `false` | no |
| <a name="input_hetzner_remove_fqdn_resolve"></a> [hetzner\_remove\_fqdn\_resolve](#input\_hetzner\_remove\_fqdn\_resolve) | if the FQDN should be removed from the entry `127.0.1.1 ...` in `/etc/hosts` | `bool` | `true` | no |
| <a name="input_hostname"></a> [hostname](#input\_hostname) | the value for 'hostname' | `string` | `null` | no |
| <a name="input_hostname_fqdn"></a> [hostname\_fqdn](#input\_hostname\_fqdn) | if cloud-init user data for hostname and FQDN should be generated | `bool` | `false` | no |
| <a name="input_ip_addresses"></a> [ip\_addresses](#input\_ip\_addresses) | the list of ip address suffixes and the commands to compute them (s. variable ip4\_address\_command) | <pre>list(object({<br>    ip_address_suffix   = string<br>    computation_command = string<br>  }))</pre> | `[]` | no |
| <a name="input_ipv4_address_command"></a> [ipv4\_address\_command](#input\_ipv4\_address\_command) | the command to determin the ipv4 address, other possible ways are<br>      - ip route get 8.8.8.8 \| grep 8.8.8.8 \| sed -E 's/.*src (\S*) .*/\1/'<br>      - ip addr show \| grep 'inet ' \| grep 'scope global' \| cut -d ' ' -f6 \| cut -d '/' -f 1 \| head -n 1<br>      - curl https://ifconfig.me | `string` | `"ip addr show | grep 'inet ' | grep 'scope global' | cut -d ' ' -f6 | cut -d '/' -f 1 | head -n 1"` | no |
...
| <a name="input_secretor"></a> [secretor](#input\_secretor) | if cloud-init user data for installing openbao or vault should be generated | `bool` | `false` | no |
| <a name="input_secretor_api_addr"></a> [secretor\_api\_addr](#input\_secretor\_api\_addr) | the [api\_addr](https://www.secretorproject.io/docs/configuration#api_addr):<br><br>    Specifies the address (full URL) to advertise to other secretor servers in the cluster for client redirection.<br>    This value is also used for plugin backends.<br>    This can also be provided via the environment variable secretor\_API\_ADDR.<br>    In general this should be set as a full URL that points to the value of the listener address.<br><br>    the string '$ipv4\_address' can be used as placeholder for the server ipv4-address | `string` | `null` | no |
| <a name="input_secretor_bootstrap_files_path"></a> [secretor\_bootstrap\_files\_path](#input\_secretor\_bootstrap\_files\_path) | the path where the files needed for bootstrapping are saved | `string` | `"/root"` | no |
| <a name="input_secretor_chown_files"></a> [secretor\_chown\_files](#input\_secretor\_chown\_files) | the list of files to be changed to ownership secretor:secretor (before starting secretor) | `list(string)` | `[]` | no |
| <a name="input_secretor_cluster_addr"></a> [secretor\_cluster\_addr](#input\_secretor\_cluster\_addr) | the [cluster\_addr](https://www.secretorproject.io/docs/configuration#cluster_addr)<br><br>    Specifies the address to advertise to other secretor servers in the cluster for request forwarding.<br>    This is a full URL, like api\_addr, but secretor will ignore the scheme (all cluster members always use TLS with a private key/certificate).<br><br>    the string '$ipv4\_address' can be used as placeholder for the server ipv4-address (determined by variable ipv4\_address\_command) | `string` | `null` | no |
| <a name="input_secretor_config_path"></a> [secretor\_config\_path](#input\_secretor\_config\_path) | the path for the secretor configuration files | `string` | `"/etc/secretor.d"` | no |
| <a name="input_secretor_disable_mlock"></a> [secretor\_disable\_mlock](#input\_secretor\_disable\_mlock) | the value for [disable\_mlock](https://www.secretorproject.io/docs/configuration#disable_mlock) | `bool` | `true` | no |
| <a name="input_secretor_helper_cmd_http_address"></a> [secretor\_helper\_cmd\_http\_address](#input\_secretor\_helper\_cmd\_http\_address) | the secretor address (http) for the helper cmds<br>    (if null the helper cmds are not installed) | `string` | `null` | no |
| <a name="input_secretor_home_path"></a> [secretor\_home\_path](#input\_secretor\_home\_path) | the home of the secretor specific files and folders | `string` | `"/srv/secretor"` | no |
| <a name="input_secretor_init"></a> [secretor\_init](#input\_secretor\_init) | if secretor should be initialized | `bool` | `true` | no |
| <a name="input_secretor_init_json_file_mode"></a> [secretor\_init\_json\_file\_mode](#input\_secretor\_init\_json\_file\_mode) | the file mode for the secretor init json result files | `string` | `"640"` | no |
| <a name="input_secretor_init_pgp_public_keys"></a> [secretor\_init\_pgp\_public\_keys](#input\_secretor\_init\_pgp\_public\_keys) | the definition of the usage of pgp keys for secretor init<br>    note: the number of pgp\_external\_public\_keys plus num\_internal\_unseal\_keys has to match secretor\_key\_shares | <pre>object({<br>    num_internal_unseal_keys = optional(number, 1),<br>    pgp_external_public_keys = optional(list(object({<br>      content  = string,<br>      encoding = optional(string, "text/plain"),<br>      owner    = optional(string, "root")<br>      group    = optional(string, "root")<br>      mode     = optional(string, "640")<br>    })), [])<br>  })</pre> | `null` | no |
| <a name="input_secretor_init_public_key"></a> [secretor\_init\_public\_key](#input\_secretor\_init\_public\_key) | the public RSA key the output of the secretor initialization is encoded with (to be decryptable by the corresponding private key with [rsadecrypt](https://developer.hashicorp.com/terraform/language/functions/rsadecrypt) | `string` | `null` | no |
| <a name="input_secretor_install_method"></a> [secretor\_install\_method](#input\_secretor\_install\_method) | the install method, supported methods are 'apt', 'package, or 'binary'<br>  - 'package' uses secretor\_package\_src<br>  - 'binary' uses secretor\_version | `string` | `"apt"` | no |
| <a name="input_secretor_key_shares"></a> [secretor\_key\_shares](#input\_secretor\_key\_shares) | the number of [key shares](https://developer.hashicorp.com/secretor/docs/commands/operator/init#key-shares) | `number` | `1` | no |
| <a name="input_secretor_key_threshold"></a> [secretor\_key\_threshold](#input\_secretor\_key\_threshold) | the number of key shares required to reconstruct the root key (s. https://developer.hashicorp.com/secretor/docs/commands/operator/init#key-threshold) | `number` | `1` | no |
| <a name="input_secretor_listeners"></a> [secretor\_listeners](#input\_secretor\_listeners) | the list of [listener](https://www.secretorproject.io/docs/configuration/listener/tcp)s<br>    the default for each (coded in terraform)<br>      - tls\_cert\_file is [secretor\_tls\_cert\_file](#input\_secretor\_tls\_cert\_file)<br>      - tls\_key\_file is [secretor\_tls\_key\_file](#input\_secretor\_tls\_key\_file)<br>      - tls\_client\_ca\_file [secretor\_tls\_client\_ca\_file](#input\_secretor\_tls\_client\_ca\_file)<br>    the string '$ipv4\_address' can be used as placeholder for the server ipv4-address in address and cluster\_adrress | <pre>list(object({<br>    address            = string,<br>    cluster_address    = optional(string, null),<br>    tls_disable        = optional(bool, true),<br>    tls_cert_file      = optional(string, null),<br>    tls_key_file       = optional(string, null),<br>    tls_client_ca_file = optional(string, null),<br>  }))</pre> | `[]` | no |
| <a name="input_secretor_local_addr"></a> [secretor\_local\_addr](#input\_secretor\_local\_addr) | the secretor address used for secretor init, secretor operator init, secretor operator unseal and secretor token revoke during cloud init | `string` | `null` | no |
| <a name="input_secretor_log_level"></a> [secretor\_log\_level](#input\_secretor\_log\_level) | the [secretor log level](https://www.secretorproject.io/docs/configuration#log_level) | `string` | `"info"` | no |
| <a name="input_secretor_package_src"></a> [secretor\_package\_src](#input\_secretor\_package\_src) | the source for the package | `string` | `null` | no |
| <a name="input_secretor_raft_leader_tls_servername"></a> [secretor\_raft\_leader\_tls\_servername](#input\_secretor\_raft\_leader\_tls\_servername) | the [leader\_tls\_servername](https://www.secretorproject.io/docs/configuration/storage/raft#leader_tls_servername) | `string` | `null` | no |
| <a name="input_secretor_raft_retry_autojoin"></a> [secretor\_raft\_retry\_autojoin](#input\_secretor\_raft\_retry\_autojoin) | the auto\_join values for [retry\_join](https://developer.hashicorp.com/secretor/docs/configuration/storage/raft#retry_join-stanza)<br>    - [auto\_join](https://developer.hashicorp.com/secretor/docs/configuration/storage/raft#auto_join)<br>    - [auto\_join\_scheme](https://developer.hashicorp.com/secretor/docs/configuration/storage/raft#auto_join\_scheme)<br>    - [auto\_join\_port](https://developer.hashicorp.com/secretor/docs/configuration/storage/raft#auto_join\_port)<br>    - computation\_command\_template: template to compute the node ip matching the ip of another node<br>    [discover](https://github.com/hashicorp/go-discover) has to be installed for secretor\_spread\_secretor\_init\_json | <pre>object({<br>    auto_join                       = string,<br>    auto_join_scheme                = optional(string, null),<br>    auto_join_port                  = optional(number, null),<br>    ip_computation_command_template = optional(string, "ip route get $ip | grep $ip | sed -E 's/.*src (\\S*) .*/\\1/'"),<br>  })</pre> | `null` | no |
| <a name="input_secretor_receive_secretor_init_json"></a> [secretor\_receive\_secretor\_init\_json](#input\_secretor\_receive\_secretor\_init\_json) | if the secretor init json result should be received from spreading | `bool` | `false` | no |
| <a name="input_secretor_remove_secretor_init_json"></a> [secretor\_remove\_secretor\_init\_json](#input\_secretor\_remove\_secretor\_init\_json) | if the output of the secretor initialization should removed<br>    <span style=\"color:red\">ATTENTION: The output of the secretor initialization is highly confidential! It is the root of the secret management in secretor!</span>" | `bool` | `true` | no |
| <a name="input_secretor_remove_spread_secretor_init_json_id_file"></a> [secretor\_remove\_spread\_secretor\_init\_json\_id\_file](#input\_secretor\_remove\_spread\_secretor\_init\_json\_id\_file) | if the ssh id file used for spreading the secretor init json result to the cluster should be removed after used | `bool` | `true` | no |
| <a name="input_secretor_revoke_root_token"></a> [secretor\_revoke\_root\_token](#input\_secretor\_revoke\_root\_token) | if the initial root token should be revoked | `bool` | `true` | no |
| <a name="input_secretor_secure_init_json"></a> [secretor\_secure\_init\_json](#input\_secretor\_secure\_init\_json) | if the output of the secretor initialization should secured<br>    <span style=\"color:red\">ATTENTION: The output of the secretor initialization is highly confidential! It is the root of the secret management in secretor!</span>" | `bool` | `true` | no |
| <a name="input_secretor_spread_secretor_init_json"></a> [secretor\_spread\_secretor\_init\_json](#input\_secretor\_spread\_secretor\_init\_json) | if the secretor init json result should be spread to the cluster | `bool` | `false` | no |
| <a name="input_secretor_spread_secretor_init_json_id_file"></a> [secretor\_spread\_secretor\_init\_json\_id\_file](#input\_secretor\_spread\_secretor\_init\_json\_id\_file) | the ssh id file used for spreading the secretor init json result to the cluster | `string` | `null` | no |
| <a name="input_secretor_start"></a> [secretor\_start](#input\_secretor\_start) | if secretor should be started | `bool` | `false` | no |
| <a name="input_secretor_storage_raft_cluster_member_this"></a> [secretor\_storage\_raft\_cluster\_member\_this](#input\_secretor\_storage\_raft\_cluster\_member\_this) | the actual instance to be excluded for the [retry\_join-stanza](https://www.secretorproject.io/docs/configuration/storage/raft#retry_join-stanza)s | `string` | `null` | no |
| <a name="input_secretor_storage_raft_cluster_members"></a> [secretor\_storage\_raft\_cluster\_members](#input\_secretor\_storage\_raft\_cluster\_members) | the list of cluster members for the [retry\_join-stanza](https://www.secretorproject.io/docs/configuration/storage/raft#retry_join-stanza)s | `list(string)` | `[]` | no |
| <a name="input_secretor_storage_raft_node_id"></a> [secretor\_storage\_raft\_node\_id](#input\_secretor\_storage\_raft\_node\_id) | the `node_id` value for `storage "raft"` | `string` | `null` | no |
| <a name="input_secretor_storage_raft_path"></a> [secretor\_storage\_raft\_path](#input\_secretor\_storage\_raft\_path) | the `path` value for `storage "raft"` | `string` | `"/srv/secretor/file/raft"` | no |
| <a name="input_secretor_storage_raft_retry_join_api_port"></a> [secretor\_storage\_raft\_retry\_join\_api\_port](#input\_secretor\_storage\_raft\_retry\_join\_api\_port) | the port number for the [leader\_api\_addr](https://developer.hashicorp.com/secretor/docs/configuration/storage/raft#leader_api_addr) in the [retry\_join-stanza](https://www.secretorproject.io/docs/configuration/storage/raft#retry_join-stanza)s | `number` | `8200` | no |
| <a name="input_secretor_tls_cert_file"></a> [secretor\_tls\_cert\_file](#input\_secretor\_tls\_cert\_file) | the path of the certificate for TLS ([tls\_cert\_file](https://www.secretorproject.io/docs/configuration/listener/tcp#tls_cert_file)<br>    default is [secretor\_storage\_raft\_leader\_client\_cert\_file](#input\_secretor\_storage\_raft\_leader\_client\_cert\_file) (coded in terraform) | `string` | `null` | no |
| <a name="input_secretor_tls_client_ca_file"></a> [secretor\_tls\_client\_ca\_file](#input\_secretor\_tls\_client\_ca\_file) | the [tls\_client\_ca\_file](https://www.secretorproject.io/docs/configuration/listener/tcp#tls_client_ca_file)<br>    default is [secretor\_storage\_raft\_leader\_ca\_cert\_file](#input\_secretor\_storage\_raft\_leader\_ca\_cert\_file) (coded in terraform) | `string` | `null` | no |
| <a name="input_secretor_tls_contents"></a> [secretor\_tls\_contents](#input\_secretor\_tls\_contents) | the secretor tls file contents<br>    tls\_file has to be one of<br>      - cert<br>      - key<br>      - client\_ca<br>      - storage\_raft\_leader\_ca\_cert<br>      - storage\_raft\_leader\_client\_cert<br>      - storage\_raft\_leader\_client\_key<br>    and the corresponding terraform variable is used as file\_name<br>  - encoding of the content can be 'text/plain' (default) or 'base64' | <pre>list(object({<br>    tls_file = optional(string, null),<br>    content  = string,<br>    encoding = optional(string, "text/plain")<br>    owner    = optional(string, "secretor")<br>    group    = optional(string, "secretor")<br>    mode     = optional(string, "640")<br>  }))</pre> | `[]` | no |
| <a name="input_secretor_tls_files"></a> [secretor\_tls\_files](#input\_secretor\_tls\_files) | DEPRECATED: use secretor\_tls\_contents instead<br>    the secretor tls files<br>    filename can contain the placeholders<br>      - $secretor\_tls\_cert\_file<br>      - $secretor\_tls\_key\_file<br>      - $secretor\_tls\_client\_ca\_file<br>    which are replace by the corresponding terraform variables<br>  - encoding of the content can be 'text/plain' (default) or 'base64' | <pre>list(object({<br>    file_name = string,<br>    content   = string,<br>    encoding  = optional(string, "text/plain")<br>    owner     = optional(string, "secretor")<br>    group     = optional(string, "secretor")<br>    mode      = optional(string, "640")<br>  }))</pre> | `[]` | no |
| <a name="input_secretor_tls_key_file"></a> [secretor\_tls\_key\_file](#input\_secretor\_tls\_key\_file) | the path of the private key for the certificate for TLS ([tls\_key\_file](https://www.secretorproject.io/docs/configuration/listener/tcp#tls_key_file))<br>    default is [secretor\_storage\_raft\_leader\_client\_key\_file](#input\_secretor\_storage\_raft\_leader\_client\_key\_file) (coded in terraform) | `string` | `null` | no |
| <a name="input_secretor_tls_storage_raft_leader_ca_cert_file"></a> [secretor\_tls\_storage\_raft\_leader\_ca\_cert\_file](#input\_secretor\_tls\_storage\_raft\_leader\_ca\_cert\_file) | the [leader\_ca\_cert\_file](https://www.secretorproject.io/docs/configuration/storage/raft#leader_ca_cert_file)<br>    default is [secretor\_home\_path](#input\_secretor\_home\_path)/tls/client\_ca.pem (coded in terraform) | `string` | `null` | no |
| <a name="input_secretor_tls_storage_raft_leader_client_cert_file"></a> [secretor\_tls\_storage\_raft\_leader\_client\_cert\_file](#input\_secretor\_tls\_storage\_raft\_leader\_client\_cert\_file) | the [leader\_client\_cert\_file](https://www.secretorproject.io/docs/configuration/storage/raft#leader_client_cert_file)<br>    default is [secretor\_home\_path](#input\_secretor\_home\_path)/tls/cert.pem (coded in terraform) | `string` | `null` | no |
| <a name="input_secretor_tls_storage_raft_leader_client_key_file"></a> [secretor\_tls\_storage\_raft\_leader\_client\_key\_file](#input\_secretor\_tls\_storage\_raft\_leader\_client\_key\_file) | the [leader\_client\_key\_file](https://www.secretorproject.io/docs/configuration/storage/raft#leader_client_key_file)<br>    default is [secretor\_home\_path](#input\_secretor\_home\_path)/tls/key.pem (coded in terraform) | `string` | `null` | no |
| <a name="input_secretor_ui"></a> [secretor\_ui](#input\_secretor\_ui) | if the secretor user interface should be activated | `bool` | `false` | no |
| <a name="input_secretor_unseal"></a> [secretor\_unseal](#input\_secretor\_unseal) | if secretor should be unsealed | `bool` | `false` | no |
| <a name="input_secretor_version"></a> [secretor\_version](#input\_secretor\_version) | the secretor version to be installed | `string` | `null` | no |
| <a name="input_secretor_zipped_binary_url"></a> [secretor\_zipped\_binary\_url](#input\_secretor\_zipped\_binary\_url) | the download url for secretor install method 'binary'<br>     - ${secretor\_version} is replaced by the value for secretor\_version<br>     - the download has to be a zip file containing the secretor binary | `string` | `"https://releases.hashicorp.com/secretor/${secretor_version}/secretor_${secretor_version}_linux_amd64.zip"` | no |
