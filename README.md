...
| <a name="input_vault_init_pgp_public_keys"></a> [vault\_init\_pgp\_public\_keys](#input\_vault\_init\_pgp\_public\_keys) | the definition of the usage of pgp keys for vault init<br>    note: the number of pgp\_public\_keys plus num\_internal\_unseal\_keys has to match vault\_key\_shares | <pre>object({<br>    encrypt_unseal_key_with_pgp_public_keys = optional(bool, true),<br>    num_internal_unseal_keys                = optional(number, 1),<br>    pgp_external_public_keys = optional(list(object({<br>      content  = string,<br>      encoding = optional(string, "text/plain"),<br>    })), [])<br>  })</pre> | `null` | no |
