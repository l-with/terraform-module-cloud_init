...
| <a name="input_ipv4_address_command"></a> [ipv4\_address\_command](#input\_ipv4\_address\_command) | the command to determin the ipv4 address | `string` | `"ip addr show | grep 'inet ' | grep 'scope global' | cut -d ' ' -f6 | cut -d '/' -f 1 | head -n 1"` | no |
